...
class TestAUSTransaction(unittest.TestCase, MemoryDatabaseMixin):
    def setUp(self):
        ...
        self.table = Table(
            "test", self.metadata, Column("id", Integer, primary_key=True), Column("foo", Integer)
        )
...
class TestTableMixin(object):
    def setUp(self):
        ...
        self.test.history.t.insert().execute(
            change_id=2, timestamp=10, changed_by="admin", id=1, foo=30, data_version=1
        )
        self.test.history.t.insert().execute(
            change_id=3, timestamp=20, changed_by="admin", id=1, foo=31, data_version=2
        )
        self.test.history.t.insert().execute(
            change_id=4, timestamp=30, changed_by="admin", id=1, foo=32, data_version=3
        )
        self.test.history.t.insert().execute(
            change_id=5, timestamp=40, changed_by="admin", id=1, foo=33, data_version=4
        )
        self.test.history.t.insert().execute(change_id=6, timestamp=9, changed_by="admin", id=2)
        self.test.history.t.insert().execute(
            change_id=7, timestamp=10, changed_by="admin", id=2, foo=18, data_version=1
        )
        self.test.history.t.insert().execute(
            change_id=8, timestamp=15, changed_by="admin", id=2, foo=19, data_version=2
        )
        self.test.history.t.insert().execute(
            change_id=9, timestamp=20, changed_by="admin", id=2, foo=20, data_version=3
        )
        self.test.history.t.insert().execute(
            change_id=10, timestamp=25, changed_by="admin", id=2, foo=21, data_version=4
        )
        self.test.history.t.insert().execute(
            change_id=11, timestamp=30, changed_by="admin", id=2, foo=22, data_version=5
        )
        self.test.history.t.insert().execute(change_id=12, timestamp=22, changed_by="admin", id=3)
        self.test.history.t.insert().execute(
            change_id=13, timestamp=23, changed_by="admin", id=3, foo=6, data_version=1
        )
        self.test.history.t.insert().execute(
            change_id=14, timestamp=26, changed_by="admin", id=3, foo=7, data_version=2
        )
        self.test.history.t.insert().execute(
            change_id=15, timestamp=29, changed_by="admin", id=3, foo=8, data_version=3
        )
        self.test.history.t.insert().execute(
            change_id=16, timestamp=32, changed_by="admin", id=3, foo=9, data_version=4
        )
        self.test.history.t.insert().execute(
            change_id=17, timestamp=35, changed_by="admin", id=3, foo=10, data_version=5
        )
        self.test.history.t.insert().execute(
            change_id=18, timestamp=38, changed_by="admin", id=3, foo=11, data_version=6
        )
        self.test.history.t.insert().execute(change_id=19, timestamp=19, changed_by="admin", id=4)
        self.test.history.t.insert().execute(
            change_id=20, timestamp=20, changed_by="admin", id=4, foo=40, data_version=1
        )
        self.test.history.t.insert().execute(
            change_id=21, timestamp=25, changed_by="admin", id=4, foo=41, data_version=2
        )
        self.test.history.t.insert().execute(
            change_id=22, timestamp=30, changed_by="admin", id=4, foo=42, data_version=3
        )
...
class TestAUSTable(unittest.TestCase, TestTableMixin, MemoryDatabaseMixin):
    ...
    def testSelect(self):
        expected = [
            dict(id=1, foo=33, data_version=4),
            dict(id=2, foo=22, data_version=5),
            dict(id=3, foo=11, data_version=6),
        ]
    ...
    def testDelete(self):
        ret = self.test.delete(
            changed_by="bill", where=[self.test.id == 1, self.test.foo == 33], old_data_version=4
        )
    ...
    def testUpdate(self):
        ret = self.test.update(
            changed_by="bob", where=[self.test.id == 1], what=dict(foo=123), old_data_version=4
        )
        self.assertEqual(ret.rowcount, 1)
    ...
    def testUpdateClosesConnectionOnImplicitTransaction(self):
        with mock.patch("sqlalchemy.engine.base.Connection.close") as close:
            self.test.update(
                changed_by="bob", where=[self.test.id == 1], what=dict(foo=432), old_data_version=4
            )
    ...
    def testUpdateWithChangeCallback(self):
        ...
        self.test.update(
            changed_by="bob",
            where=where,
            what=dict(chain(what.items(), additional_columns.items())),
            old_data_version=4,
        )
        # update adds data_version and id to the query, so we need to add that before comparing
        what["data_version"] = 5
...
class TestAUSTableRequiresRealFile(unittest.TestCase, TestTableMixin, NamedFileDatabaseMixin):
    ...
    def testDeleteWithTransaction(self):
        trans = AUSTransaction(self.metadata.bind.connect())
        self.test.delete(
            changed_by="bill", transaction=trans, where=[self.test.id == 2], old_data_version=5
        )
        ret = self.test.t.select().execute().fetchall()
...
class TestHistoryTable(unittest.TestCase, TestTableMixin, MemoryDatabaseMixin):
    ...
    def testHistoryUponAutoincrementInsert(self):
        ...
        ret = (
            self.test.history.t.select().where(self.test.history.id == 4).execute().fetchall()[-2:]
        )
    ...
    def testHistoryUponUpdate(self):
        self.test.update(
            changed_by="heather", where=[self.test.id == 2], what=dict(foo=99), old_data_version=5
        )
    ...
    def testHistoryTimestampMaintainsPrecision(self):
        ...
        ret = (
            select([self.test.history.timestamp])
            .where(self.test.history.id == 5)
            .execute()
            .fetchone()[0]
        )
    ...
    def testHistoryGetChangeWithDataVersionWithNonPrimaryKeyColumn(self):
        self.test.insert(changed_by="george", id=5, foo=0)
        self.assertRaises(
            ValueError, self.test.history.getChange, data_version=1, column_values={"foo": 5}
        )
...
class TestMultiplePrimaryHistoryTable(
    unittest.TestCase, TestMultiplePrimaryTableMixin, MemoryDatabaseMixin
):
    def setUp(self):
        MemoryDatabaseMixin.setUp(self)
    ...
    def testMultiplePrimaryHistoryUponInsert(self):
        ...
        self.assertEqual(
            ret, [(1, "george", 999, 4, 5, None, None), (2, "george", 1000, 4, 5, 0, 1)]
        )

    @mock.patch("time.time", mock.MagicMock(return_value=1.0))
    def testMultiplePrimaryHistoryUponDelete(self):
        self.test.delete(
            changed_by="bobby", where=[self.test.id1 == 1, self.test.id2 == 2], old_data_version=1
        )
    ...
    def testMultiplePrimaryHistoryUponUpdate(self):
        self.test.update(
            changed_by="heather",
            where=[self.test.id1 == 2, self.test.id2 == 1],
            what=dict(foo=99),
            old_data_version=1,
        )
        ret = self.test.history.t.select().execute().fetchone()
        ...
        class TestTable(AUSTable):
            def __init__(self, db, metadata):
                ...
                super(TestTable, self).__init__(
                    db, "sqlite", scheduled_changes=True, versioned=True, historyClass=HistoryTable
                )
            ...
            def update(
                ...
                for row in self.select(where=where, transaction=transaction):
                    if not self.db.hasPermission(
                        ...
                        raise PermissionDeniedError("fail")
                if not dryrun:
                    super(TestTable, self).update(
                        where, what, changed_by, old_data_version, transaction, dryrun
                    )
            ...
            def delete(
                ...
                if not dryrun:
                    super(TestTable, self).delete(
                        where, changed_by, old_data_version, transaction, dryrun
                    )
...
class TestScheduledChangesTable(unittest.TestCase, ScheduledChangesTableMixin, MemoryDatabaseMixin):
    ...
    def testValidateConditionsNone(self):
        self.assertRaisesRegex(
            ValueError, "No conditions found", self.sc_table.conditions.validate, {}
        )

    def testValidateConditionsNoneValue(self):
        self.assertRaisesRegex(
            ValueError, "No conditions found", self.sc_table.conditions.validate, {"when": None}
        )

    def testValdiateConditionsInvalid(self):
        self.assertRaisesRegex(
            ValueError, "Invalid condition", self.sc_table.conditions.validate, {"blah": "blah"}
        )
    ...
    def testValidateConditionsBadWhen(self):
        self.assertRaisesRegex(
            ValueError, "Cannot parse", self.sc_table.conditions.validate, {"when": "abc"}
        )
    ...
    def testInsertForExistingRow(self):
        ...
        cond_row = (
            self.sc_table.conditions.t.select()
            .where(self.sc_table.conditions.sc_id == 7)
            .execute()
            .fetchall()[0]
        )
    ...
    def testInsertWithNonAutoincrement(self):
        class TestTable2(AUSTable):
            def __init__(self, db, metadata):
                self.table = Table(
                    "test_table2",
                    metadata,
                    Column("foo_name", String(15), primary_key=True),
                    Column("foo", String(15)),
                    Column("bar", String(15)),
                )
                super(TestTable2, self).__init__(
                    db, "sqlite", scheduled_changes=True, versioned=True
                )
        ...
        sc_row = (
            table.scheduled_changes.t.select()
            .where(table.scheduled_changes.sc_id == 1)
            .execute()
            .fetchall()[0]
        )
        cond_row = (
            table.scheduled_changes.conditions.t.select()
            .where(table.scheduled_changes.conditions.sc_id == 1)
            .execute()
            .fetchall()[0]
        )
    ...
    def testInsertWithNonNullablePKColumn(self):
        class TestTable(AUSTable):
            def __init__(self, db, metadata):
                ...
                super(TestTable, self).__init__(
                    db, "sqlite", scheduled_changes=True, historyClass=None, versioned=True
                )
        ...
        self.assertRaisesRegex(
            ValueError, "Missing primary key column ", table_sc.insert, changed_by="alice", **what
        )
    ...
    def testInsertWithMalformedTimestamp(self):
        ...
        self.assertRaisesRegex(
            ValueError, "Cannot parse", self.sc_table.insert, changed_by="bob", **what
        )
    ...
    def testInsertDataVersionChanged(self):
        ...
        self.table.update(
            [self.table.fooid == 3], what={"foo": "bb"}, changed_by="bob", old_data_version=2
        )
        what = {
            "fooid": 3,
            "data_version": 2,
    ...
    def testInsertRaisesErrorOnDataVersionBetweenCoreAndConditions(self):
        ...
        what = {"foo": "newthing1", "when": 888000, "change_type": "insert"}
        self.assertRaises(
            MismatchedDataVersionError, self.sc_table.insert, changed_by="bob", **what
        )
    ...
    def testUpdateNoChangesSinceCreation(self):
        ...
        sc_history_row = (
            self.sc_table.history.t.select()
            .where(self.sc_table.history.sc_id == 1)
            .execute()
            .fetchall()[0]
        )
        cond_row = (
            self.sc_table.conditions.t.select()
            .where(self.sc_table.conditions.sc_id == 1)
            .execute()
            .fetchall()[0]
        )
        cond_history_row = (
            self.sc_table.conditions.history.t.select()
            .where(self.sc_table.conditions.history.sc_id == 1)
            .execute()
            .fetchall()[0]
        )
        self.assertEqual(sc_row.scheduled_by, "bob")
    ...
    def testUpdateRaisesErrorOnDataVersionBetweenCoreAndConditions(self):
        ...
        self.sc_table.conditions.t.update().where(self.sc_table.conditions.sc_id == 1).execute(
            data_version=4
        )
        self.assertRaises(
            MismatchedDataVersionError,
            self.sc_table.update,
            [self.sc_table.sc_id == 1],
            what={"bar": "bar"},
            changed_by="bob",
            old_data_version=1,
        )
    ...
    def testUpdateBaseTableNoConflictWithChanges(self):
        ...
        # value.
        self.table.update(
            [self.table.fooid == 2], what={"bar": "bar"}, changed_by="bob", old_data_version=2
        )
        ...
        sc_history_row = (
            self.sc_table.history.t.select()
            .where(self.sc_table.history.sc_id == 4)
            .execute()
            .fetchall()[0]
        )
        cond_row = (
            self.sc_table.conditions.t.select()
            .where(self.sc_table.conditions.sc_id == 4)
            .execute()
            .fetchall()[0]
        )
        cond_history_row = (
            self.sc_table.conditions.history.t.select()
            .where(self.sc_table.conditions.history.sc_id == 4)
            .execute()
            .fetchall()[0]
        )
    ...
    def testUpdateBaseTableNoConflictChangingToNull(self):
        ...
        # value.
        self.table.update(
            [self.table.fooid == 2], what={"bar": None}, changed_by="bob", old_data_version=2
        )
        row = self.table.t.select().where(self.table.fooid == 2).execute().fetchall()[0]
        sc_row = self.sc_table.t.select().where(self.sc_table.sc_id == 4).execute().fetchall()[0]
        sc_history_row = (
    ...
    def testUpdateDeleteScheduledChange(self):
        ...
        sc_history_row = (
            self.sc_table.history.t.select()
            .where(self.sc_table.history.sc_id == 6)
            .execute()
            .fetchall()[0]
        )
        cond_row = (
            self.sc_table.conditions.t.select()
            .where(self.sc_table.conditions.sc_id == 6)
            .execute()
            .fetchall()[0]
        )
        cond_history_row = (
            self.sc_table.conditions.history.t.select()
            .where(self.sc_table.conditions.history.sc_id == 6)
            .execute()
            .fetchall()[0]
        )
        self.assertEqual(sc_row.scheduled_by, "bob")
    ...
    def testDeleteChange(self):
        ...
        ret = (
            self.sc_table.conditions.t.select()
            .where(self.sc_table.conditions.sc_id == 2)
            .execute()
            .fetchall()
        )
        self.assertEqual(len(ret), 0)
    ...
    def testEnactChangeNewRow(self):
        ...
        history_rows = (
            self.table.history.t.select().where(self.table.history.fooid == 5).execute().fetchall()
        )
    ...
    def testEnactChangeExistingRow(self):
        ...
        history_row = (
            self.table.history.t.select()
            .where(self.table.history.fooid == 1)
            .where(self.table.history.data_version == 2)
            .execute()
            .fetchall()[0]
        )
        sc_row = self.sc_table.t.select().where(self.sc_table.sc_id == 1).execute().fetchall()[0]
    ...
    def testEnactChangeNoPermissions(self):
        ...
        self.assertRaises(
            PermissionDeniedError, self.table.scheduled_changes.enactChange, 1, "jeremy"
        )
    ...
    def testEnactInsertsInConditionsHistory(self):
        ...
        num_history_row = (
            self.sc_table.history.t.select()
            .where(self.sc_table.history.sc_id == 2)
            .execute()
            .fetchall()
        )
        num_cond_history_row = (
            self.sc_table.conditions.history.t.select()
            .where(self.sc_table.conditions.history.sc_id == 2)
            .execute()
            .fetchall()
        )
    ...
    def testMergeUpdateWithConflict(self):
        ...
        self.assertRaises(
            UpdateMergeError, self.sc_table.mergeUpdate, old_row, what, changed_by="bob"
        )

    @mock.patch("time.time", mock.MagicMock(return_value=200))
    def testMergeDontChangeScheduledby(self):
        self.table.update(
            [self.table.fooid == 2], what={"bar": "bar1"}, changed_by="mary", old_data_version=2
        )
...
class TestScheduledChangesWithConfigurableConditions(unittest.TestCase, MemoryDatabaseMixin):
    ...
    def testSCTableWithNoConditions(self):
        class TestTable2(AUSTable):
            def __init__(self, db, metadata):
                ...
                super(TestTable2, self).__init__(
                    ...
                    "sqlite",
                    scheduled_changes=True,
                    scheduled_changes_kwargs={"conditions": []},
                    versioned=True,
                )
        ...
        self.assertRaisesRegex(
            ValueError, "No conditions enabled", TestTable2, self.db, self.metadata
        )
        ...
        class TestTable3(AUSTable):
            def __init__(self, db, metadata):
                ...
                super(TestTable3, self).__init__(
                    ...
                    "sqlite",
                    scheduled_changes=True,
                    scheduled_changes_kwargs={"conditions": ["time", "blech"]},
                    versioned=True,
                )
    ...
    def testUpdateChangeToDisabledCondition(self):
        where = [self.sc_table.sc_id == 1]
        what = {
            "telemetry_product": "pro",
            "telemetry_channel": "cha",
            "telemetry_uptake": 3456,
            "bar": "ccc",
            "when": None,
        }
        self.assertRaisesRegex(
...
class TestSignoffsTable(unittest.TestCase, MemoryDatabaseMixin):
    ...
    def testSignoffWithPermission(self):
        ...
        got = (
            self.signoffs.t.select()
            .where(self.signoffs.sc_id == 1)
            .where(self.signoffs.username == "bob")
            .execute()
            .fetchall()
        )
    ...
    def testSignoffASecondTimeWithSameRole(self):
        ...
        got = (
            self.signoffs.t.select()
            .where(self.signoffs.sc_id == 1)
            .where(self.signoffs.username == "nancy")
            .execute()
            .fetchall()
        )
        self.assertEqual(got, [(1, "nancy", "relman")])
        history = (
            self.signoffs.history.t.select()
            .where(self.signoffs.sc_id == 1)
            .where(self.signoffs.username == "nancy")
            .execute()
            .fetchall()
    ...
    def testCannotUpdateSignoff(self):
        self.assertRaises(
            AttributeError, self.signoffs.update, {"username": "nancy"}, {"role": "qa"}, "nancy"
        )
...
class TestProductRequiredSignoffsTable(unittest.TestCase, MemoryDatabaseMixin):
    def setUp(self):
        ...
        self.rs.t.insert().execute(
            product="foo", channel="bar", role="releng", signoffs_required=1, data_version=1
        )
        self.rs.t.insert().execute(
            product="foo", channel="bar", role="relman", signoffs_required=2, data_version=1
        )
        self.rs.t.insert().execute(
            product="apple", channel="orange", role="releng", signoffs_required=2, data_version=1
        )
    ...
    def testInsertNewRequiredSignoffWithSpecificPermission(self):
        self.rs.insert(
            changed_by="bob", product="carrot", channel="celery", role="releng", signoffs_required=1
        )
        got = self.rs.t.select().where(self.rs.product == "carrot").execute().fetchall()
        self.assertEqual(got, [("carrot", "celery", "releng", 1, 1)])
    ...
    def testInsertRequiredSignoffWithScheduledChange(self):
        ...
        got = (
            self.rs.t.select()
            .where(self.rs.product == "foo")
            .where(self.rs.channel == "bar")
            .where(self.rs.role == "qa")
            .execute()
            .fetchall()
        )
    ...
    def testInsertRequiredSignoffWithoutEnoughSignoffs(self):
        self.assertRaises(
            SignoffRequiredError, self.rs.scheduled_changes.enactChange, sc_id=4, enacted_by="bill"
        )
...
class TestPermissionsRequiredSignoffsTable(unittest.TestCase, MemoryDatabaseMixin):
    def setUp(self):
        ...
        self.rs.t.insert().execute(
            product="foo", role="releng", signoffs_required=1, data_version=1
        )
        self.rs.t.insert().execute(
            product="foo", role="relman", signoffs_required=2, data_version=1
        )
        self.rs.t.insert().execute(
            product="apple", role="releng", signoffs_required=2, data_version=1
        )
        self.rs.scheduled_changes.t.insert().execute(
            sc_id=1,
            scheduled_by="bob",
...
class TestRulesSimple(unittest.TestCase, RulesTestMixin, MemoryDatabaseMixin):
    ...
    def testGetOrderedRulesWithCondition(self):
        rules = self._stripNullColumns(
            self.paths.getOrderedRules(where=[self.paths.buildTarget == "d"])
        )
        expected = [
            dict(
                rule_id=4,
    ...
    def testAddRule(self):
        what = dict(
            backgroundRate=11, mapping="c", update_type="z", priority=60, product="a", channel="a"
        )
    ...
    def testUpdateRuleByAlias(self):
        ...
        what["mapping"] = "d"
        self.paths.update(
            where={"rule_id": "radagast"}, what=what, changed_by="bill", old_data_version=1
        )
...
class TestReleases(unittest.TestCase, MemoryDatabaseMixin):
    def setUp(self):
        ...
        self.releases.t.insert().execute(
            name="ab",
            product="a",
            data=createBlob(dict(name="ab", schema_version=1, hashFunction="sha512")),
            data_version=1,
        )
        self.releases.t.insert().execute(
            name="b",
        ...
        self.releases.t.insert().execute(
            name="c",
            product="c",
            data=createBlob(dict(name="c", schema_version=1, hashFunction="sha512")),
            data_version=1,
        )
        self.releases.t.insert().execute(
            name="h",
            product="b",
            data=createBlob(dict(name="h", schema_version=1, hashFunction="sha512")),
            data_version=1,
        )
        ...
        self.permissions.t.insert().execute(
            permission="release", username="bob", options=dict(products=["c"]), data_version=1
        )
    ...
    def testGetReleaseInfoAll(self):
        ...
        expected = [
            ...
            dict(
                name="ab", product="a", data_version=1, read_only=False, rule_ids=[], rule_info={}
            ),
            ...
            dict(
                ...
                read_only=False,
                rule_ids=[2],
                rule_info={"2": {"product": "b", "channel": "h"}},
            ),
            dict(
                name="h",
                product="b",
                ...
                rule_ids=[1, 2],
                rule_info={
                    "1": {"product": "b", "channel": "h"},
                    "2": {"product": "b", "channel": "h"},
                },
            ),
        ]
    ...
    def testGetReleaseInfoWithFallbackMapping(self):
        ...
        expected = [
            dict(
                ...
                read_only=False,
                rule_ids=[4],
                rule_info={"4": {"product": None, "channel": None}},
            )
        ]
        self.assertEqual(releases, expected)
    ...
    def testDeleteWithRuleMapping(self):
        self.releases.t.insert().execute(
            name="d",
            product="d",
            data=createBlob(dict(name="d", schema_version=1, hashFunction="sha512")),
            data_version=1,
        )
        self.rules.t.insert().execute(
            rule_id=4,
        ...
        self.assertRaises(
            ValueError, self.releases.delete, {"name": "d"}, changed_by="me", old_data_version=1
        )
    ...
    def testDeleteReleaseWhenReadOnly(self):
        self.releases.t.update(values=dict(read_only=True, data_version=2)).where(
            self.releases.name == "a"
        ).execute()
        self.assertRaises(
            ReadOnlyError, self.releases.delete, {"name": "a"}, changed_by="me", old_data_version=2
        )
    ...
    def testAddReleaseWithNameMismatch(self):
        ...
        self.assertRaises(
            ValueError, self.releases.insert, "bill", name="g", product="g", data=blob
        )

    def testUpdateReleaseNoPermissionForNewProduct(self):
        self.assertRaises(
            PermissionDeniedError, self.releases.update, {"name": "c"}, {"product": "d"}, "bob", 1
        )

    def testUpdateReleaseWithNameMismatch(self):
        newBlob = ReleaseBlobV1(name="c", schema_version=1, hashFunction="sha512")
        self.assertRaises(
            ValueError, self.releases.update, {"name": "a"}, {"data": newBlob}, "bill", 1
        )
    ...
    def testUpdateReleaseChangeReadOnly(self):
        ...
        self.assertEqual(
            select([self.releases.read_only])
            .where(self.releases.name == "a")
            .execute()
            .fetchone()[0],
            True,
        )
    ...
    def testUpdateReleaseWithRuleMappingThatRequiresSignoff(self):
        ...
        self.assertRaises(
            SignoffRequiredError, self.releases.update, {"name": "h"}, {"data": newBlob}, "bill", 1
        )
    ...
    def testAddLocaleToReleaseUpdatesCaches(self):
        with mock.patch("time.time") as t:
            ...
            newBlob = {
                ...
                "name": "b",
                "hashFunction": "sha512",
                "platforms": {"win": {"locales": {"zu": {"buildID": 123}}}},
            }
    ...
    def testAddRelease(self):
        ...
        expected = [
            (
                ...
                "d",
                False,
                createBlob(dict(name="d", schema_version=1, hashFunction="sha512")),
                1,
            )
        ]
        self.assertEqual(
            self.releases.t.select().where(self.releases.name == "d").execute().fetchall(), expected
        )
    ...
    def testUpdateRelease(self):
        ...
        expected = [
            (
                ...
                "z",
                False,
                createBlob(dict(name="a", schema_version=1, hashFunction="sha512")),
                2,
            )
        ]
        self.assertEqual(
            self.releases.t.select().where(self.releases.name == "a").execute().fetchall(), expected
        )
    ...
    def testUpdateReleaseWithBlob(self):
        ...
        expected = [
            (
                ...
                "z",
                False,
                createBlob(dict(name="b", schema_version=1, hashFunction="sha512")),
                2,
            )
        ]
        self.assertEqual(
            self.releases.t.select().where(self.releases.name == "b").execute().fetchall(), expected
        )
...
"""
        ...
        with self.db.begin() as trans:
            self.releases.insert(
                changed_by="bill", name="p", product="z", data=ancestor_blob, transaction=trans
            )
            self.releases.update(
                {"name": "p"},
                {"product": "z", "data": blob1},
                changed_by="bill",
                old_data_version=1,
                transaction=trans,
            )
        ...
        history_entries = [
            blob.data
            for name, blob in self.releases.history.bucket.blobs.items()
...
"""
        ...
        ret = (
            select([self.releases.data])
            .where(self.releases.name == "release4")
            .execute()
            .fetchone()[0]
        )
        self.assertEqual(result_blob, ret)
...
class TestPermissions(unittest.TestCase, MemoryDatabaseMixin):
    def setUp(self):
        ...
        self.permissions.t.insert().execute(
            permission="release", username="bob", options=dict(products=["fake"]), data_version=1
        )
        self.permissions.t.insert().execute(
            permission="release", username="janet", options=dict(products=["fake"]), data_version=1
        )
        self.permissions.t.insert().execute(permission="rule", username="cathy", data_version=1)
        self.permissions.t.insert().execute(
            permission="rule", username="bob", options=dict(actions=["modify"]), data_version=1
        )
        self.permissions.t.insert().execute(
            permission="rule",
            username="fred",
            options=dict(products=["foo", "bar"], actions=["modify"]),
            data_version=1,
        )
        self.permissions.t.insert().execute(
            permission="admin", username="george", options=dict(products=["foo"]), data_version=1
        )
    ...
    def testGrantPermissions(self):
        self.permissions.insert(
            "bob", username="cathy", permission="release", options=dict(products=["SeaMonkey"])
        )
        ...
        self.assertEqual(
            query.execute().fetchall(), [("release", "cathy", dict(products=["SeaMonkey"]), 1)]
        )

    def testGrantPermissionsUnknownPermission(self):
        self.assertRaises(
            ValueError, self.permissions.insert, changed_by="bob", username="bud", permission="bad"
        )
    ...
    def testGrantRoleWithPermission(self):
        ...
        got = (
            self.user_roles.t.select()
            .where(self.user_roles.username == "fred")
            .execute()
            .fetchall()
        )
    ...
    def testGrantRoleForExistingUser(self):
        ...
        got = (
            self.user_roles.t.select().where(self.user_roles.username == "bob").execute().fetchall()
        )
    ...
    def testRevokePermission(self):
        self.permissions.delete(
            {"username": "bob", "permission": "release"}, changed_by="bill", old_data_version=1
        )
    ...
    def testRevokePermissionThatDoesntSupportProductOption(self):
        self.assertRaises(
            SignoffRequiredError,
            self.permissions.delete,
            {"username": "sean", "permission": "permission"},
            changed_by="bill",
            old_data_version=1,
        )
    ...
    def testRevokingPermissionAlsoRevokeRoles(self):
        self.permissions.delete(
            {"username": "janet", "permission": "release"}, changed_by="bill", old_data_version=1
        )
        got = (
            self.db.permissions.t.select()
            .where(self.db.permissions.username == "janet")
            .execute()
            .fetchall()
        )
        self.assertEqual(len(got), 0)
        got = (
            self.user_roles.t.select()
            .where(self.user_roles.username == "janet")
            .execute()
            .fetchall()
        )
        self.assertEqual(len(got), 0)
    ...
    def testRevokePermissionWithProductThatRequiresSignoff(self):
        self.assertRaises(
            SignoffRequiredError,
            self.permissions.delete,
            {"username": "george", "permission": "admin"},
            changed_by="bill",
            old_data_version=1,
        )
    ...
    def testRevokePermissionWithoutProductThatRequiresSignoff(self):
        self.assertRaises(
            SignoffRequiredError,
            self.permissions.delete,
            {"username": "cathy", "permission": "rule"},
            changed_by="bill",
            old_data_version=1,
        )
    ...
    def testGetUserRoles(self):
        ...
        expected = sorted(
            [{"data_version": 1, "role": "releng"}, {"data_version": 1, "role": "dev"}],
            key=lambda k: k["role"],
        )
...
class TestChangeNotifiers(unittest.TestCase):
    def setUp(self):
        ...
        self.metadata.create_all(self.db.engine)
        self.db.rules.t.insert().execute(
            rule_id=2,
            priority=100,
            channel="release",
            backgroundRate=100,
    ...
    def testOnInsertRule(self):
        ...
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", RegexPartialString(r"'channel':\s[b|u]?'bar'")
        )
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", PartialString("'rule_id':")
        )

    def testOnInsertPermission(self):
        def doit():
            self.db.permissions.insert(
                "bob", permission="admin", username="charlie", data_version=1
            )
        ...
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", PartialString("'permission': 'admin'")
        )
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", PartialString("'username': 'charlie'")
        )
    ...
    def testOnUpdate(self):
        ...
        mock_conn.sendmail.assert_any_call(
            "fake@from.com", "fake@to.com", RegexPartialString(r"'channel':\s[b|u]?\"?'release'\"?")
        )
        mock_conn.sendmail.assert_called_with(
        ...
        mock_conn.sendmail.assert_called_with(
            "fake@from.com",
            "fake@to.com",
            RegexPartialString(r"'base_product':\s[b|u]?\"?None ---> 'blah'\"?"),
        )
        mock_conn.sendmail.assert_called_with(
            "fake@from.com",
            "fake@to.com",
            RegexPartialString(r"'base_channel':\s[u|b]?\"?'release'\"?,"),
        )
    ...
    def testOnInsertRuleSC(self):
        ...
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", RegexPartialString(r"'scheduled_by':\s[u|b]?'bob'")
        )
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", RegexPartialString(r"'base_channel':\s[u|b]?'bar'")
        )
    ...
    def testOnUpdateRuleSC(self):
        ...
        mock_conn.sendmail.assert_called_with(
            "fake@from.com",
            "fake@to.com",
            RegexPartialString(r"'base_channel':\s[b|u]?\"?'release'\"?"),
        )
    ...
    def testOnDeleteRuleSC(self):
        ...
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", RegexPartialString(r"'base_channel':\s[b|u]?'release'")
        )

    def testOnChangeReadOnly(self):
        def doit():
            self.db.releases.update(
                {"name": "a"}, {"read_only": False}, changed_by="bob", old_data_version=1
            )
        ...
        mock_conn.sendmail.assert_called_with(
            "fake@from.com",
            "fake@to.com",
            RegexPartialString("Read only release" r"\s[b|u]?'a' changed to modifiable"),
        )
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", RegexPartialString(r"'name':\s[b|u]?'a'")
        )
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", RegexPartialString(r"'product':\s[b|u]?'a'")
        )
        mock_conn.sendmail.assert_called_with(
            "fake@from.com", "fake@to.com", PartialString("'read_only': True" " ---> False")
        )

    def testOnChangeReadOnlySetUnmodifiable(self):
        def doit():
            self.db.releases.update(
                {"name": "b"}, {"read_only": False}, changed_by="bob", old_data_version=1
            )
...
class TestDBModel(unittest.TestCase, NamedFileDatabaseMixin):
    ...
    def _rules_version_length_migration_test(self, db, upgrade=True):
        ...
        if upgrade:
            ...
            for table_name in tables_list:
                self.assertEqual(
                    downgrade_length, meta_data.tables[table_name].c.version.type.length
                )
            for table_name in scheduled_changes_tables:
                self.assertEqual(
                    downgrade_length, meta_data.tables[table_name].c.base_version.type.length
                )
    ...
    def _test_rules_longer_distribution(self, db, upgrade=True):
        ...
        if upgrade:
            for table_name in tables_list:
                self.assertEqual(
                    upgraded_length, meta_data.tables[table_name].c.distribution.type.length
                )
            for table_name in scheduled_changes_tables:
                self.assertEqual(
                    upgraded_length, meta_data.tables[table_name].c.base_distribution.type.length
                )
        else:
            for table_name in tables_list:
                self.assertEqual(
                    downgrade_length, meta_data.tables[table_name].c.distribution.type.length
                )
            for table_name in scheduled_changes_tables:
                self.assertEqual(
                    downgrade_length, meta_data.tables[table_name].c.base_distribution.type.length
                )
