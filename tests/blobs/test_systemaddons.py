...
""")
    ...
    def _makeBlob(self, addon="a", version="5", hashFunction="SHA512", hashValue="50" * 64):
        blob = SystemAddonsBlob()
        blob.loadJSON(
            json.dumps(
                {
    ...
    def testAddonNameRejectsXMLMetacharacters(self):
        # The addon name is interpolated verbatim into an XML attribute, so it
        # must not be allowed to contain characters that break out of it.
        for addon in ['a" URL="http://evil.com/x', "a<b", "a>b", "a&b"]:
            blob = self._makeBlob(addon=addon)
