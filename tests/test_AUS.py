...
class TestAUSThrottlingWithoutFallback(unittest.TestCase):
            ...
            data=createBlob(
                {
                    ...
                    "extv": "1.0",
                    "schema_version": 1,
                    "platforms": {"a": {"buildID": "1", "locales": {"a": {}}}},
                }
            ),
        )
    ...
    def testThrottling25WithForcingFailure(self):
        (served, fallback, tested) = self.random_aus_test(
            background_rate=25, force=FORCE_FALLBACK_MAPPING
        )
    ...
    def testThrottling100WithFallback(self):
        (served_mapping, served_fallback, tested) = self.random_aus_test(
            background_rate=100, fallback=True
        )
    ...
    def testThrottling50WithFallback(self):
        (served_mapping, served_fallback, tested) = self.random_aus_test(
            background_rate=50, fallback=True
        )
    ...
    def testThrottling25WithFallback(self):
        (served_mapping, served_fallback, tested) = self.random_aus_test(
            background_rate=25, fallback=True
        )
    ...
    def testThrottlingZeroWithFallback(self):
        (served_mapping, served_fallback, tested) = self.random_aus_test(
            background_rate=0, fallback=True
        )
    ...
    def testThrottling25WithForcingAndFallback(self):
        (served_mapping, served_fallback, tested) = self.random_aus_test(
            background_rate=25, force=FORCE_MAIN_MAPPING, fallback=True
        )
    ...
    def testThrottling25WithForcingFailureAndFallback(self):
        (served, fallback, tested) = self.random_aus_test(
            background_rate=25, force=FORCE_FALLBACK_MAPPING, fallback=True
        )
