...
class ClientTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Ripped from
        # https://github.com/mitsuhiko/flask/blob/1f5927eee2288b4aaf508af5dc1f148aa2140d91/flask/app.py#L394
...
""")
    ...
    def testGetHeaderArchitectureWindows(self):
        self.assertEqual(
            self.view.getHeaderArchitecture(
                'WINNT_x86-msvc',
                'Firefox Intel Windows'),
            'Intel')
    ...
    def testDontUpdateToYourself(self):
        ...
        self.assertEqual(minidom.parseString(ret.data).getElementsByTagName(
            'updates')[0].firstChild.nodeValue, '\n')
    ...
    def testDontDecreaseVersion(self):
        ret = self.client.get(
            '/update/3/c/15.0/1/p/l/a/a/default/a/update.xml')
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(ret.mimetype, 'text/xml')
        self.assertEqual(minidom.parseString(ret.data).getElementsByTagName(
            'updates')[0].firstChild.nodeValue, '\n')
    ...
    def testVersion1Get(self):
        ...
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(ret.mimetype, 'text/xml')
        # We need to load and re-xmlify these to make sure we don't get
        # failures due to whitespace differences.
...
""")
    ...
    def testVersion3Get(self):
        ...
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(ret.mimetype, 'text/xml')
        # An empty update contains an <updates> tag with a newline, which is
...
""")
    ...
    def testBadAvastURLsFromBug1125231(self):
        ...
        ret = self.client.get(
            '/update/4/b/1.0/1/p/x86 l/a/a/a/a/1/update.xml')
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(ret.mimetype, 'text/xml')
...
""")
    ...
    def testAvastURLsWithBadQueryArgs(self):
        ret = self.client.get(
            "/update/4/b/1.0/1/p/l/a/a/a/a/1/update.xml?force=1%3Favast=1")
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(ret.mimetype, 'text/xml')
        ret2 = self.client.get(
            '/update/4/b/1.0/1/p/l/a/a/a/a/1/update.xml?force=1')
        self.assertEqual(ret2.status_code, 200)
        self.assertEqual(ret2.mimetype, 'text/xml')
        self.assertEqual(ret.data, ret2.data)
    ...
    def testAvastURLsWithUnescapedBadQueryArgs(self):
        ret = self.client.get(
            "/update/4/b/1.0/1/p/l/a/a/a/a/1/update.xml?force=1?avast=1")
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(ret.mimetype, 'text/xml')
        ret2 = self.client.get(
            '/update/4/b/1.0/1/p/l/a/a/a/a/1/update.xml?force=1')
...
""")
    ...
    def testDeprecatedEsrVersionStyleGetsUpdates(self):
        ret = self.client.get(
            '/update/3/b/1.0esrpre/1/p/l/a/a/a/a/update.xml')
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(ret.mimetype, 'text/xml')
        # We need to load and re-xmlify these to make sure we don't get
        # failures due to whitespace differences.
...
class ClientTestWithErrorHandlers(unittest.TestCase):
    ...
    def testEmptySnippetOn500(self):
        with mock.patch('auslib.web.views.client.ClientRequestView.get') as m:
            ...
            ret = self.client.get(
                '/update/4/b/1.0/1/p/l/a/a/a/a/1/update.xml')
            self.assertEqual(ret.status_code, 200)
            self.assertEqual(ret.mimetype, 'text/xml')
            self.assertEqual(minidom.parseString(ret.data).getElementsByTagName(
                'updates')[0].firstChild.nodeValue, '\n')
