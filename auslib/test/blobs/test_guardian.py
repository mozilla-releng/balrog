...
@pytest.mark.parametrize(
    "whitelistedDomains,expected",
    [
        ({"a.com": ("Guardian",)}, False),
        ({}, True),
    ],
)
def testContainsForbiddenDomain(guardblob, whitelistedDomains, expected):
    assert guardblob.containsForbiddenDomain("Guardian", whitelistedDomains) == expected
...
def testShouldServeUpdateVariousVersions(guardblob, version, expected):
    ...
    assert guardblob.shouldServeUpdate(updateQuery) == expected
...
def testShouldServeUpdateMissingBuildTarget(guardblob):
    ...
    assert guardblob.shouldServeUpdate(updateQuery) == False
...
@pytest.mark.parametrize(
    ...
    [
        (
            "WINNT_x86_64",
            {"a.com": ("Guardian",)},
            {"required": True, "url": "https://a.com/this/is/1.0.0.0.msi", "version": "1.0.0.0"}
        ),
        (
            "Darwin_x86_64",
            {"a.com": ("Guardian",)},
            {"required": True, "url": "https://a.com/this/is/1.0.0.0.dmg", "version": "1.0.0.0"}
        ),
        (
            "Linux_x86_64",
            {"a.com": ("Guardian",)},
