...
class TestPermissionsAPI_JSON(ViewTest, JSONTestMixin):
    ...
    def testPermissionPut(self):
        ...
        self.assertEqual(query.execute().fetchone(),
                         ('admin', 'bob', None, 1))
    ...
    def testPermissionsPost(self):
        ...
        self.assertEqual(
            ...
            "Data: %s" %
            ret.data)
        r = dbo.permissions.t.select().where(
            dbo.permissions.username == 'bill').execute().fetchall()
        self.assertEqual(len(r), 1)
        self.assertEqual(r[0], ('admin', 'bill', None, 2))

    def testPermissionsPostMissing(self):
        ret = self._post(
            "/users/bill/permissions/rules/:id",
            data=dict(
                options="",
                data_version=1))
    ...
    def testPermissionUrl(self):
        ...
        self.assertEqual(query.execute().fetchone(),
                         ('/releases/:name', 'cathy', None, 1))

    def testPermissionPutWithOption(self):
        ret = self._put(
            '/users/bob/permissions/rules',
            data=dict(
                options=json.dumps(
                    dict(
                        product='fake'))))
    ...
    def testPermissionModify(self):
        ret = self._put(
            '/users/bob/permissions/releases/:name',
            data=dict(
                options=json.dumps(
                    dict(
                        product='different')),
    ...
    def testPermissionDelete(self):
        ...
        query = dbo.permissions.t.select()
        query = query.where(dbo.permissions.username == 'bob')
        query = query.where(dbo.permissions.permission ==
