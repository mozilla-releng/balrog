...
"""))
    ...
    def testReleasePostCreatesNewReleasev1(self):
        ...
        ret = dbo.releases.t.select().where(
            dbo.releases.name == 'e').execute().fetchone()
...
"""))
    ...
    def testLocalePut(self):
        ...
        self.assertEqual(
            ...
            ret.data)
        ret = select([dbo.releases.data]).where(
            dbo.releases.name == 'a').execute().fetchone()[0]
        self.assertEqual(json.loads(ret), json.loads("""
{
    "name": "a",
    "schema_version": 1,
...
"""))
        newVersion = select([dbo.releases.version]).where(
            dbo.releases.name == 'a').execute().fetchone()[0]
    ...
    def testLocaleRevertsPartialUpdate(self):
        ...
        with mock.patch('auslib.global_state.dbo.releases.addLocaleToRelease') as r:
            ...
            ret = dbo.releases.t.select().where(
                dbo.releases.name == 'a').execute().fetchone()
            self.assertEqual(ret['product'], 'a')
            self.assertEqual(ret['version'], 'a')
            self.assertEqual(
                json.loads(
                    ret['data']), dict(
                    name='a', hashFunction="sha512", schema_version=1))
...
"""))
        ...
        self.assertEquals(
            ret.status_code, 201, "Status Code: %d, Data: %s" %
