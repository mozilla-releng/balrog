...
from .forms import ScheduledChangeNewRuleForm
...
class ScheduledChangesView(AdminView):
    ...
    def _post(self, form, transaction, changed_by):
        ...
        # The form is checked if its change_type field value is insert
        # so that it can be initially saved in the rules tables in order to
        # generate a rule_id before being saved in the schedule change table.
        # The form also was checked if its an instance of the
        # ScheduledChangeNewRuleForm class, so as to ensure that only
        # those form class instances will be affected.
        if columns["change_type"] == "insert" and isinstance(form, ScheduledChangeNewRuleForm):
            for key in ["when", "change_type"]:
                # keys where removed temporarily to ensure only the necessary
                # fields are available when inserting into the rules table
                del columns[key]
            rule_id = dbo.rules.insert(changed_by=changed_by, **columns)
            # ensures all required fields needed for insertion into the
            # schedule table are available back after the initial temporary
            # removal above
            columns = {k: v.data for k, v in form._fields.iteritems()}
            # adds the rule_id value generated by the rule table
            columns["rule_id"] = int(rule_id)
        sc_id = self.sc_table.insert(changed_by, transaction, **columns)
        # returns the rule_id along with the sc_id in the response for
        # forms with both fields present in them
        if "rule_id" in columns:
            return jsonify(sc_id=sc_id, rule_id=columns["rule_id"])
        # returns only the schedule id for forms without rule_id
