...
def compare_snippets(url1, url2, retries=3, timeout=10, diff=True):
    ...
    xml1 = retry(
        requests.get, sleeptime=5, attempts=retries, args=(
            url1,), retry_exceptions=(
            requests.HTTPError, requests.ConnectionError), kwargs={'timeout': timeout, 'config': cfg})
    xml1 = xml1.content.splitlines()
    xml2 = retry(
        requests.get, sleeptime=5, attempts=retries, args=(
            url2,), retry_exceptions=(
            requests.HTTPError, requests.ConnectionError), kwargs={'timeout': timeout, 'config': cfg})
