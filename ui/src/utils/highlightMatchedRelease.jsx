...
export default (highlights, releaseName) => {
  // The highlights array elements are arrays with pairs of indices indicating:
  ...
  if (highlights) {
    ...
    for (let i = 1; i < highlights.length; i += 1) {
      ...
      if (highlights[i + 1]) {
        ...
        // and there are remaining characters in releaseName,
        // add all remaining characters
        const isLastSubstring = highlights[i][1] < releaseName.length;

        if (isLastSubstring)
          highlightedName.push(releaseName.slice(highlights[i][1]));
      }
    }
