  ...
  describe("Filters", function() {
    it("should return the response when 'rules' object is selected", function() {
      ...
      this.$httpBackend
        ...
        .respond(200, '{"rules": [], "count": 0}');
      this.$httpBackend
        .expectGET("/api/rules/history?&page=1")
        .respond(
          200,
          JSON.stringify({
            Rules: { count: 2, revisions: ["test", "test2"] },
    ...
    it("should return the response when the changed_by name is entered", function() {
      ...
      this.scope.userInput.dateRangeStart = 1508923005671;
      this.scope.userInput.dateRangeEnd = 1508923005724;
      this.scope.searchHistory();
      var parent = this;
      this.$httpBackend
        .expectGET("/api/rules")
      ...
      this.$httpBackend
        .expectGET(
          "/api/rules/history?&page=1&changed_by=testemail1@mozilla.com&timestamp_from=1508923005671&timestamp_to=1508923005724"
        )
      ...
      expect(this.scope.searchResult[0]).toEqual(result[0][0]);
      // expect(this.scope.searchResult).toEqual(['test','test2']);
    });
